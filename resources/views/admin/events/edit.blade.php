...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("event.update", [$monthMi->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('image') ? 'is-invalid' : '' }}" value = "{{$monthMi->image}}" type="hidden" name="prev_photo" id="image" >
                <img width = "200px" height = "200px" src = "{{asset('storage/'.$monthMi->image)}}">
                @if($errors->has('image'))
