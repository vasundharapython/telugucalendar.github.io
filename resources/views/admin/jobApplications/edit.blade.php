    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.job-applications.update", [$jobApplication->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('job') ? 'is-invalid' : '' }}" type="text" name="job" id="job" value="{{ old('job', $jobApplication->job) }}">
                @if($errors->has('job'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('job_profile') ? 'is-invalid' : '' }}" name="job_profile_id" id="job_profile_id">
                    @foreach($job_profiles as $id => $entry)
                        <option value="{{ $id }}" {{ (old('job_profile_id') ? old('job_profile_id') : $jobApplication->job_profile->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $jobApplication->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone_no') ? 'is-invalid' : '' }}" type="text" name="phone_no" id="phone_no" value="{{ old('phone_no', $jobApplication->phone_no) }}">
